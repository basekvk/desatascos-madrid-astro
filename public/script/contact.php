...
$dzEmailTo 		= "info@grupalsl.es";   /* Receiver Email Address */
$dzEmailFrom    = "Desatascos-Madrid.com";
...
try {
    if (!empty($_POST)) {
        ...
        if (!$response->isSuccess()) {
            $dzRes['status'] = 0;
			$dzRes['msg'] = 'ReCaptcha no ha sido validado.';
			echo json_encode($dzRes);
			exit;
        }
		...
		if($_POST['dzToDo'] == 'Contact')
		{
			...
			$dzMailSubject = 'Desatascos Madrid Web';
			$dzMailMessage	= 	"
								Alguien quiere contactar: <br><br>
								Name: $dzName<br/>
								Email: $dzEmail<br/>
			...
			{
				$dzRes['status'] = 1;
				$dzRes['msg'] = 'Hemos recibido tu mensaje correctamente. Gracias por contactar';
			}
			else
			{
				$dzRes['status'] = 0;
				$dzRes['msg'] = 'Ha habido algún problema al contactar. Intentalo de nuevo por 
				favor';
			}
			echo json_encode($dzRes);
			exit;
		...
		if($_POST['dzToDo'] == 'Appointment')
		{
			...
			$dzMailSubject = 'Destascos Madrid Web Nuevo contacto';
			$dzMailMessage	= 	"
								Nuevo Contacto: <br><br>
								Name: $dzName<br/>
								Email: $dzEmail<br/>
								Message: $dzMessage<br/>
								";
			...
			{
				$dzRes['status'] = 1;
				$dzRes['msg'] = 'Hemos recibido tu mensaje perfectamente. Gracias por contactar';
			}
			else
			{
				$dzRes['status'] = 0;
				$dzRes['msg'] = 'Ha habido algún problema al contactar. Vuelve a intentarlo.';
			}
			echo json_encode($dzRes);
			exit;
		}	
    ...
    $dzRes['status'] = 0;
	$dzRes['msg'] = $e->getMessage(). 'Ha habido algún problema al contactar. Vuelve a intentarlo.';
	echo json_encode($dzRes);
	exit;
}
